...
- "Working tree clean" was true at the moment stated, but re-running the parity/
  ...
  dirty again on the next `git status`. **Fixed at the root**: these derived files are
  no longer git-tracked (`.gitignore`; see commit on `fix/report-gaps-and-ship-order`)
  — they're regenerated by `run-tests.js` and asserted on directly, nothing depends on
