...
### A6 — the post-merge reminder (closing §6's own gap)

§6 recorded that "nothing prompts anyone to run `check-installs` after a merge. The
mechanism exists; the habit does not." `.githooks/post-merge` closes that, wired by the
existing `make install-hooks`.

Three properties, each learned from a failure in this repository:

1. **It never blocks.** A post-merge hook cannot abort a merge (the merge already
   happened), but it must not fail loudly or corrupt a scripted pull either. It always
   exits 0, and `check-installs` is invoked with `|| true` — verified by deleting
   `check-installs.mjs` and confirming the merge still exits 0 with the hook's own lines
   intact.
2. **It never fires when nothing changed.** A reminder on every pull is a reminder nobody
   reads — the same reasoning that keeps `check-installs` out of the gate. It fires only
   when the merge changed `skill-manifest.json`, which the gate guarantees moves if and
   only if a shipped file moved.
3. **It answers rather than nags.** It runs the check and prints the result, so the common
   case costs one quiet line instead of a chore.

**A defect this exposed.** The manifest recorded `git_commit`, which moves with HEAD, so it
was rewritten on every commit and the file was perpetually dirty. A file that is always
modified is a file whose diffs stop being read. `--write` now leaves the manifest untouched
when `skill_version` is unchanged, so only real content changes produce a diff.

Falsified across five cases: a docs-only merge is silent; a skill-changing merge fires and
names the version delta; a **registered install going stale reports `STALE` with both
versions** (the F-72 scenario, end to end); a missing `check-installs.mjs` does not break
the pull; and a plain merge with no skill change stays silent.
...
- **The post-merge hook only helps someone who ran `make install-hooks`.** A fresh clone has
  no hooks until it does, and `install.sh` does not wire them. That is a smaller version of
  the same gap — the mechanism exists, reaching for it is still a habit — and it is narrower
  than what A6 closed, since a clone that has never installed the skill has nothing to go
  stale.
