...
//
// SC-14 (this change) adds `gates[].failure_reason`, the top-level `terminal_failure_reason`
// and `failure_severity` report fields, and one new NO_RETRY_REASONS member
// (`EVIDENCE_INTEGRITY_BREACH`) — all additive, same gate keys, same DECISIONS, same exit
// codes. Every prior additive change on this list bumped SCHEMA_VERSION; that bump is
// DEFERRED here because parity/execution-report-fixtures/run-tests.js pins `'1.4.0'` by
// exact string match (see its schema_version assertion) and `parity/` is a blocked path
// for this job's contract, so the bump and its fixture update cannot land in the same
// change. Left as `1.4.0` on purpose — bump to `1.5.0` and update that assertion together
// the next time a job can touch `parity/`.
...
const NO_RETRY_REASONS = new Set([
  ...
  'ADVOCATE_DISSENT',
  // SC-14: an evidence-integrity breach (a tree entry that exists but cannot be trusted —
  // unreadable, or a skill_trace verdict that disagrees with its own validator output) is
  // non-retriable for the same reason CREDENTIAL_FAILURE and MISSING_UPSTREAM_ARTIFACT
  // are: retrying the same phase cannot fix evidence that already lies. Purely additive —
  // no existing member's meaning changes, and `decideLifecycle` is untouched: this reason
  // routes through the SAME branch (line ~907) that already handles every other member of
  // this Set, with the same "evidence preserved for review" wording.
  'EVIDENCE_INTEGRITY_BREACH',
]);

const QUARANTINE_REASONS = new Set(['CREDENTIAL_FAILURE', 'MISSING_UPSTREAM_ARTIFACT']);

// SC-14: the frozen precedence order used to pick ONE `terminal_failure_reason` when more
// than one gate carries a non-null `failure_reason` in the same report (e.g. `skills_clean`
// and `consensus` both fail). Integrity is named FIRST, matching the SC-8/F-55 rule already
// applied elsewhere in this file: an evidence-integrity breach outranks the verdict it was
// hiding, because a dissent recorded against a tree that also contains a forged or
// unreadable file cannot be trusted to mean what it says. Exported so a doc or a check can
// assert against the order rather than re-deriving it.
const TERMINAL_FAILURE_REASON_PRECEDENCE = Object.freeze(['EVIDENCE_INTEGRITY_BREACH', 'ADVOCATE_DISSENT']);

// SC-14: severity is a PROJECTION of the two Sets above, not a second hand-maintained
// catalog — so the classification `decideLifecycle` reads and the severity a report reader
// sources are computed from the same objects and cannot drift apart. Returns
// 'non-retriable' when `reason` is a member of NO_RETRY_REASONS or QUARANTINE_REASONS,
// 'retriable' for any other non-empty string, and `null` for null/undefined/''.
function failureReasonSeverity(reason) {
  if (typeof reason !== 'string' || reason.length === 0) return null;
  if (NO_RETRY_REASONS.has(reason) || QUARANTINE_REASONS.has(reason)) return 'non-retriable';
  return 'retriable';
}
...
function evalConsensus(consensusRows, supersededDissentRows) {
  ...
  if (blocking.length > 0) {
    ...
    return {
      ...
      // SC-14: the reason as a FIELD, not just a substring of `reason`/`detail` above —
      // this is what lets a reader source ADVOCATE_DISSENT without parsing prose. Covers
      // both an unresolved dissent and one the operator explicitly UPHELD; the gate above
      // already treats them identically.
      failure_reason: 'ADVOCATE_DISSENT',
    };
  }
...
function evalSkillsClean(skillVerdicts, supersededMismatches) {
  ...
  if (evidenceProblems.length > 0) {
    ...
    return {
      ...
      // SC-14: the reason as a FIELD — see the ADVOCATE_DISSENT comment above for why.
      failure_reason: 'EVIDENCE_INTEGRITY_BREACH',
    };
  }
...
function renderMarkdown(report, phaseAttemptRows) {
  ...
  lines.push(`- **Terminal failure reason:** ${report.terminal_failure_reason || '—'}`);
  lines.push(`- **Failure severity:** ${report.failure_severity || '—'}`);
...
function buildReport(jobDir, options = {}) {
  ...
  // SC-14: `terminal_failure_reason` is the first gate-level `failure_reason` found by
  // scanning `gates` in the frozen TERMINAL_FAILURE_REASON_PRECEDENCE order, falling back
  // to `run_manifest.failure_reason` (`failureReason` above) when no gate carries one — the
  // ordinary {PHASE}_GATE_FAILURE / budget-exhaustion case. `failure_severity` is that
  // reason's severity, sourced from `failureReasonSeverity` (itself a projection of
  // NO_RETRY_REASONS / QUARANTINE_REASONS) rather than re-derived from any `detail` string.
  const terminalFailureReason =
    TERMINAL_FAILURE_REASON_PRECEDENCE.find((candidate) =>
      gates.some((g) => g.failure_reason === candidate)
    ) || failureReason;
  const failureSeverity = failureReasonSeverity(terminalFailureReason);
  ...
  const report = {
    ...
    // SC-14: additive — sourced from the same scan/projection computed above, never
    // re-derived by a reader parsing `gates[].detail`.
    ...
    gates: gates.map((g) => ({
      ...
      // SC-14: additive — null on every gate except the two non-retriable conditions that
      // set it above (consensus's blocking-dissent branch, skills_clean's two integrity
      // branches). This is what turns the reason from a substring of `detail` into a field.
      failure_reason: g.failure_reason || null,
    })),
...
module.exports = {
  ...
  QUARANTINE_REASONS,
  SCHEMA_VERSION,
  // SC-14: severity sourcing surface — see the constant/function definitions above.
  failureReasonSeverity,
  TERMINAL_FAILURE_REASON_PRECEDENCE,
};
