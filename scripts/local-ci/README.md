...
|---|---|---|---|
| 1 — host gate | `make local-ci` | yes (pre-push) | parity 93 + report 21 + entropy 7 fixtures; SC-3 provenance fixtures 3 + contract micro-drill; `node --check`; `shellcheck`+`bash -n`; SKILL.md frontmatter lint; extended NFR-4 built-ins scan. Seconds, zero-LLM. |
