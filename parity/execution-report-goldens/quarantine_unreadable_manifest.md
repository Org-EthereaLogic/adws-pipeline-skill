...
| skills_clean | fail | 1 unreadable evidence file(s) — EVIDENCE INTEGRITY: 1 evidence file(s) present but unreadable — <evidence_root>/test/attempt_1/consensus/advocate.json (unreadable: EACCES). An unreadable file is not an absent one: absence is recorded as a gap, but a file that exists and cannot be parsed is a tampering-shaped fact the report must not score as clean. |
